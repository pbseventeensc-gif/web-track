...
export default function AdminPromoManager({ isDarkMode }) {
  ...
  return (
    <div className={`p-6 rounded-3xl shadow-sm space-y-6 border ${isDarkMode ? 'bg-neutral-800 border-neutral-700 text-white' : 'bg-white border-[#D8D2C2] text-stone-800'}`}>
      ...
      {/* Form Buat Promo */}
      <div className={`p-6 rounded-3xl border shadow-sm sticky top-4 z-20 backdrop-blur-md ${isDarkMode ? 'bg-neutral-900/95 border-neutral-700 text-white' : 'bg-stone-50/95 border-stone-300 text-stone-800'}`}>
        <div className="flex justify-between items-center mb-4">
          ...
          <button 
            ...
            className="text-[11px] font-bold px-3 py-1.5 rounded-xl bg-stone-200 dark:bg-neutral-800 hover:opacity-80 transition-all"
          >
            {isEditingBudgetNames ? '💾 Selesai Edit Nama & Nominal' : '✏️ Edit Nama & Nominal Budget'}
          </button>
        </div>
        ...
        <form onSubmit={handleCreatePromo} className="space-y-4 text-xs">
          <div>
            ...
            <input 
              ...
              className={`w-full p-3 border rounded-xl font-semibold focus:outline-none ${isDarkMode ? 'bg-neutral-800 border-neutral-700 text-white' : 'bg-white border-stone-300 text-black'}`} 
            />
          </div>

          {isEditingBudgetNames && (
            <div className={`p-4 rounded-2xl border space-y-3 ${isDarkMode ? 'bg-neutral-800 border-neutral-700' : 'bg-white border-stone-300'}`}>
                ...
                {budgets.map((b, idx) => (
                  <div key={b.key} className="space-y-1">
                    ...
                    <input 
                      ...
                      className={`w-full p-2 border rounded-lg font-bold ${isDarkMode ? 'bg-neutral-900 border-neutral-600 text-white' : 'bg-stone-50 border-stone-300 text-black'}`}
                    />
                    ...
                    <input 
                      ...
                      className={`w-full p-2 border rounded-lg font-mono font-bold text-emerald-600 dark:text-emerald-400 ${isDarkMode ? 'bg-neutral-900 border-neutral-600' : 'bg-stone-50 border-stone-300'}`}
                    />
                  </div>
                ))}
              </div>
                ...
                <button
                  ...
                  className={`py-3 px-3 rounded-xl font-bold border transition-all text-center flex flex-col items-center justify-center gap-1 ${
                    form.budget_type === b.name 
                      ...
                      : isDarkMode ? 'bg-neutral-800 border-neutral-700 text-neutral-300' : 'bg-white border-stone-300 text-stone-700'
                  }`}
                >
          ...
          <div>
            ...
            <input 
              ...
              className={`w-full p-3 border rounded-xl font-mono font-bold focus:outline-none ${isDarkMode ? 'bg-neutral-800 border-neutral-700 text-emerald-400' : 'bg-white border-stone-300 text-emerald-600'}`} 
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className={`max-h-48 overflow-y-auto border p-3 rounded-xl space-y-2 ${isDarkMode ? 'bg-neutral-800 border-neutral-700' : 'bg-white border-stone-300'}`}>
              {masterItems.length === 0 ? (
                <p className="text-xs opacity-60 text-center py-2">Belum ada master barang.</p>
              ) : (
                masterItems.map(item => (
                  <label key={item.id} className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors ${selectedItemIds.includes(item.id) ? (isDarkMode ? 'bg-indigo-950/50 border border-indigo-800' : 'bg-indigo-50 border border-indigo-200') : (isDarkMode ? 'hover:bg-neutral-700' : 'hover:bg-stone-50')}`}>
          ...
          <div>
            ...
            <textarea 
              ...
              className={`w-full p-3 border rounded-xl focus:outline-none ${isDarkMode ? 'bg-neutral-800 border-neutral-700 text-white' : 'bg-white border-stone-300 text-black'}`} 
              rows="3" 
            />
          </div>
      ...
      <div className={`p-6 rounded-3xl border shadow-sm ${isDarkMode ? 'bg-neutral-900 border-neutral-700 text-white' : 'bg-stone-50 border-stone-300 text-stone-800'}`}>
        <h3 className="font-extrabold text-sm tracking-wide uppercase text-indigo-600 dark:text-indigo-400 mb-4">
        ...
        {promos.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {promos.map(p => (
              <div key={p.id} className={`p-5 border rounded-2xl flex flex-col justify-between gap-3 text-xs shadow-sm transition-all ${isDarkMode ? 'bg-neutral-800 border-neutral-700' : 'bg-white border-stone-300'}`}>
                <div>
                ...
                <div className="pt-3 border-t border-stone-200 dark:border-neutral-700 flex items-center justify-between gap-2 flex-wrap">
